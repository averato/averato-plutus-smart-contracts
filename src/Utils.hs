...
import           Cardano.Ledger.BaseTypes    (certIxToInt, txIxToInt)
...
stakeReferenceLedgerToPlutus (StakeRefPtr (Ptr (SlotNo x) y z)) = Just $ StakingPtr (fromIntegral x) (fromIntegral . txIxToInt $ y) (fromIntegral . certIxToInt $ z)
